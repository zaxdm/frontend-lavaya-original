'use client';
// app/admin/clientes/page.tsx — Vista de clientes para admin (solo lectura)
// Sin inhabilitar cuentas, sin crear usuarios. Muestra estado activo/inactivo.
...
export default function AdminClientesPage() {
  ...
  // Búsqueda usando el admin API para incluir clientes inactivos también
  const buscarClientes = useCallback(async (q: string) => {
    if (q.trim().length < 2) { setSugeridos([]); return; }
    setBuscando(true);
    try {
      const res = await adminApi.getUsuarios({ buscar: q, rol: 'CLIENTE', limit: '20' });
      setSugeridos(res.usuarios.map(u => ({
        id: u.id,
        nombre: u.nombre,
        apellido: u.apellido,
        email: u.email,
  ...
  const notasRecientes = useMemo(() =>
    historial.flatMap(p => [p.notasCliente, p.notasInternas].filter(Boolean) as string[]).slice(0, 4),
  [historial]);
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', backgroundColor: 'var(--bg-base)' }}>
      <PageHeader
        title="Clientes"
        subtitle="Consulta el historial, direcciones y membresía de cualquier cliente"
      />
                  ...
                  <button key={c.id} type="button" onClick={() => cargarCliente(c)}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                      <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)' }}>
                        {c.nombre} {c.apellido}
                        {!c.activo && (
                          <span style={{ marginLeft: 8, fontSize: 11, color: '#ef4444', fontWeight: 500 }}>Inactivo</span>
                        )}
                      </span>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 16, backgroundColor: 'var(--bg-muted)', borderRadius: 12, border: '1px solid var(--border)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start' }}>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                      ...
                      <span style={{
                        ...
                        backgroundColor: clienteSel.activo ? 'rgba(34,197,94,0.1)' : 'rgba(239,68,68,0.1)',
                        color: clienteSel.activo ? '#16a34a' : '#dc2626',
                      }}>
                        {clienteSel.activo
                          ? <><CheckCircle style={{ width: 11, height: 11 }} /> Activo</>
                          : <><XCircle    style={{ width: 11, height: 11 }} /> Inactivo</>}
                      </span>
                    </div>
                  ...
                  <button type="button" onClick={limpiar}
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 32, height: 32, borderRadius: 10, border: 'none', backgroundColor: 'rgba(37,99,235,0.08)', color: 'var(--text-secondary)', cursor: 'pointer', flexShrink: 0 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 12 }}>
                  {[
                    { label: 'Direcciones',       value: loadingDir  ? '…' : direcciones.length },
                    { label: 'Pedidos registrados', value: loadingHist ? '…' : historial.length },
                    { label: 'Membresía',
              ...
              <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 22 }}>
                ...
                <div>
                  ...
                  {loadingHist ? (
                    ...
                    <p style={{ margin: 0, fontSize: 13, color: 'var(--text-secondary)' }}>Este cliente no tiene pedidos registrados.</p>
                  ) : (
                      ...
                      {pedidosRecientes.map(pedido => (
                        <div key={pedido.id} style={{ padding: 16, borderRadius: 12, backgroundColor: 'var(--bg-muted)', border: '1px solid var(--border)' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center' }}>
                            ...
                            <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-primary)', backgroundColor: 'rgba(37,99,235,0.1)', padding: '5px 10px', borderRadius: 999 }}>
                ...
                <div>
                  ...
                  {loadingMem ? (
                    ...
                    <div style={{ padding: 14, borderRadius: 12, backgroundColor: planActivo?.bg ?? 'var(--bg-muted)', border: `1px solid ${planActivo?.color ?? 'var(--border)'}30` }}>
                    ...
                    <p style={{ margin: 0, fontSize: 13, color: 'var(--text-secondary)' }}>Sin membresía activa.</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {loadingHist ? (
                    ...
                    <p style={{ margin: 0, fontSize: 13, color: 'var(--text-secondary)' }}>No hay notas registradas.</p>
                  ) : (
            ...
            <div style={S.card}>
              <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 18 }}>
                ...
                {loadingDir ? (
                  ...
                  <p style={{ margin: 0, fontSize: 13, color: 'var(--text-secondary)' }}>Sin direcciones guardadas.</p>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {direcciones.map(d => (
                      <div key={d.id} style={{ padding: 14, borderRadius: 12, backgroundColor: 'var(--bg-muted)', border: '1px solid var(--border)', display: 'grid', gap: 8 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <Home style={{ width: 16, height: 16, color: '#2563eb' }} />
          ...
          <div style={S.card}>
            <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <p style={{ margin: 0, fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.6 }}>
                Aquí puedes consultar el historial, direcciones y membresía activa de cualquier cliente registrado, incluyendo cuentas inactivas.
              </p>
            </div>
          </div>
        )}
