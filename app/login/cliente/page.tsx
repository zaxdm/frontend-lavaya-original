...
function PasoDireccion({ dir, setDir, onBack, onConfirm, loading }: {
  ...
  // Geocodificación directa (buscar dirección escrita)
...
export default function ClienteLoginPage() {
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <button 
              onClick={() => {
                ...
                // We shouldn't call hooks in callbacks, but we can use the window object or router if we have it in scope
                // Actually we don't have router in scope. Let's just use window.location.href
                window.location.href = `/login/cliente/verificar-email?email=${encodeURIComponent(email)}`;
              }}
  ...
  const confirmarRegistro = async (omitirDireccion = false) => {
    ...
    try {
      ...
      // 2. Si hay dirección, guardarla en sessionStorage para enviarla tras verificar email
      if (!omitirDireccion && (dir.lat || dir.calle)) {
        ...
        sessionStorage.setItem('pendingDireccion', JSON.stringify(dirPayload));
      }

      toast.success('¡Registro exitoso! Verifica tu correo.');
      // 3. Redirigir a la pantalla de verificación
      window.location.href = `/login/cliente/verificar-email?email=${encodeURIComponent(rEmail)}`;
    } catch (err: unknown) {
  ...
  const resetMode = (m: Mode) => { setMode(m); setRegStep(1); setDir(DIR_VACIA); };
              ...
              {mode === 'register' && regStep === 1 && (
                <div style={{ backgroundColor: 'var(--bg-card)', borderRadius: 20, padding: 28, boxShadow: '0 32px 64px rgba(0,0,0,0.4)', border: '1px solid var(--border)' }}>
                  ...
                  <form onSubmit={irAPasoDireccion} style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
                    ...
                    <div style={{ position: 'relative' }}>
                      <Mail style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', width: 13, height: 13, color: 'var(--text-hint)' }} />
                      <input type="email" required value={rEmail} onChange={e => setREmail(e.target.value)} placeholder="Correo electrónico" style={INPUT} className="login-input" />
                    </div>
                    <div style={{ position: 'relative' }}>
                    ...
                    <button type="submit"
                      style={{ padding: '12px', borderRadius: 10, border: 'none', backgroundColor: '#0ea5e9', color: '#fff', fontSize: 14, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginTop: 2 }}>
                ...
                <div style={{ backgroundColor: 'var(--bg-card)', borderRadius: 20, padding: 32, boxShadow: '0 32px 64px rgba(0,0,0,0.4)', border: '1px solid var(--border)' }}>
                  ...
                  {fStep === 2 && (
                    <>
                      ...
                      <p style={{ fontSize: 13, color: 'var(--text-secondary)', margin: '0 0 20px', lineHeight: 1.5 }}>Ingresa el código de 6 dígitos que te enviamos.</p>
